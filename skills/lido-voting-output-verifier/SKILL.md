...
Use the registry's canonical contract name only when the address matches. First identify replacement pairs: two registry entries with the same canonical contract name, where one address is marked `[proposed to remove]` and the other address is marked `[proposed]`. Only for such a pair, require `old` immediately before the canonical contract name for the former address and `new` immediately before it for the latter address. If only one of those markers exists for a canonical contract name, require neither prefix. Treat the markers as address-specific, and never prefix an unmarked entry. Preserve checksum address casing from the parsed input when available; otherwise use the registry checksum casing if available.
...
If a contract name does not match the registry address, replace the name with the canonical registry name when matched, or mark the address `[UNKNOWN]` when unmatched. For a replacement pair with the same canonical contract name, repair the `[proposed to remove]` address label to `old <canonical contract name>` and the `[proposed]` address label to `new <canonical contract name>`. If only one marker exists for that canonical contract name, remove either prefix; also remove either prefix from unmarked entries. For a call whose calldata is parsed as `[empty]`, remove the unverified canonical name if necessary but do not add `[UNKNOWN]`; retain the raw address and any supplied parsed label.
